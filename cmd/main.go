...
import (
	...
	"github.com/ruziba3vich/cpp_service/internal/service"
...
func main() {
	fx.New(
		fx.Provide(
			...
			NewNetListener,
			NewLogger,
			service.NewCppExecutorServer,
		),
		fx.Invoke(func(lc fx.Lifecycle, lis net.Listener, server *service.CppExecutorServer) {
			s := grpc.NewServer()
