...
func NewCppClient(sessionID string, ctx context.Context, cancel context.CancelFunc, cfg *config.Config, logger *logger.Logger) *CppClient {
	return &CppClient{
		sessionID: sessionID,
...
func (c *CppClient) SendResponse(resp *compiler_service.ExecuteResponse) {
	if c.ctx.Err() != nil {
...
func (c *CppClient) SendResponses(stream compiler_service.CodeExecutor_ExecuteServer) {
...
func (c *CppClient) HandleInput(input string) {
	c.mu.Lock()
	stdin := c.stdin
	c.mu.Unlock()
...
func (c *CppClient) cleanupContainerFiles() {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	containerSrcPath := filepath.Join(c.cfg.ContainerWorkingDir, c.cfg.CppSourceFileName)
	containerExecPath := filepath.Join(c.cfg.ContainerWorkingDir, c.cfg.CppExecutableName)

	rmCmdStr := fmt.Sprintf("rm -f %s %s", containerSrcPath, containerExecPath)
	cmd := exec.CommandContext(ctx, "docker", "exec", c.cfg.CppContainerName, "sh", "-c", rmCmdStr)

	c.logger.Info("Attempting to clean up container files (cpp source and executable)", map[string]any{
		"session_id": c.sessionID,
		"command":    strings.Join(cmd.Args, " "),
	})

	if output, err := cmd.CombinedOutput(); err != nil {
		c.logger.Warn("Failed to clean up container files (or files did not exist)", map[string]any{
			"session_id": c.sessionID,
			"error":      err.Error(),
...
func (c *CppClient) ReadOutputs(stdout, stderr io.Reader) {
	defer c.wg.Done()
...
func (c *CppClient) ExecuteCpp(code string) {
	defer c.Cleanup()
	defer close(c.done)
	defer func() {
		time.Sleep(50 * time.Millisecond)
		close(c.sendChan)
		c.logger.Info("Send channel closed at the end of ExecuteCpp", map[string]any{"session_id": c.sessionID})
	}()

	c.logger.Info("Starting ExecuteCpp", map[string]any{"session_id": c.sessionID})

	tempFile, err := os.CreateTemp("", fmt.Sprintf("cpp-%s-*.cpp", c.sessionID))
	if err != nil {
	...
	copyCmd := exec.CommandContext(copyCtx, "docker", "cp", tempFilePath, fmt.Sprintf("%s:%s", c.cfg.CppContainerName, containerSrcPath))
	c.logger.Info(fmt.Sprintf("Copying %s to %s:%s", tempFilePath, c.cfg.CppContainerName, containerSrcPath),
		map[string]any{"session_id": c.sessionID, "source": tempFilePath, "destination_container": c.cfg.CppContainerName, "destination_path": containerSrcPath})
	...
	containerExecPath := filepath.Join(c.cfg.ContainerWorkingDir, c.cfg.CppExecutableName)
	compileCmd := exec.CommandContext(compileCtx, "docker", "exec", c.cfg.CppContainerName,
		"g++", "-o", containerExecPath, containerSrcPath)
	...
	if compileErr != nil {
		if errors.Is(compileErr, context.DeadlineExceeded) {
			...
			if c.ctx.Err() != nil {
				...
				c.logger.Error(fmt.Sprintf("Compilation failed unexpectedly: %v, Output: %s", compileErr, compileOutput),
	...
	if len(compileOutput) > 0 {
		c.logger.Info(fmt.Sprintf("Compilation successful with output/warnings: %s", compileOutput),
	...
	runCmd := exec.CommandContext(execCtx, "docker", "exec", "-i",
		c.cfg.CppContainerName, containerExecPath)

	c.logger.Info("Preparing C++ execution command", map[string]any{
		"session_id": c.sessionID,
	...
	c.logger.Info("Starting C++ execution", map[string]any{"session_id": c.sessionID})
	c.SendResponse(&compiler_service.ExecuteResponse{
		SessionId: c.sessionID,
	...
	if err := runCmd.Start(); err != nil {
		c.logger.Error(fmt.Sprintf("Failed to start C++ execution command: %v", err), map[string]any{"session_id": c.sessionID, "error": err})
		c.SendResponse(&compiler_service.ExecuteResponse{
			SessionId: c.sessionID,
	...
	c.logger.Info(fmt.Sprintf("C++ execution started (PID %d if available)", pid), map[string]any{"session_id": c.sessionID, "pid": pid})

	c.logger.Info("Waiting for C++ execution to complete", map[string]any{"session_id": c.sessionID})
	waitErr := runCmd.Wait()

	c.logger.Info(fmt.Sprintf("C++ execution command finished (Wait err: %v). Waiting for output processing.", waitErr),
	...
	if waitErr != nil {
		if errors.Is(waitErr, context.Canceled) {
			...
			if c.ctx.Err() != nil {
				...
				c.SendResponse(&compiler_service.ExecuteResponse{
					SessionId: c.sessionID,
					Payload:   &compiler_service.ExecuteResponse_Status{Status: &compiler_service.Status{State: "RUNTIME_ERROR"}}, // General runtime error
				})
			}
	...
	c.logger.Info("ExecuteCpp finished", map[string]any{"session_id": c.sessionID})
}

// CtxDone checks if the client's context has been cancelled.
func (c *CppClient) CtxDone() bool {
	select {
	case <-c.ctx.Done():
		return true
	default:
